...
use User as UserModel, Consumer as ConsumerModel;
use Offer as OfferModel, UserOffer as UserOfferModel;
use Loyalty as LoyaltyModel, UserLoyalty as UserLoyaltyModel;
use CommonFunction as CommonFunctionModel;
use Company as CompanyModel;
use Group as GroupModel, Member as MemberModel, MarketingHistory as MarketingHistoryModel;
...
class UserController extends \BaseController {
    ...
    public function index() {        
        ...
        return View::make('company.user.index')->with($param);
    }
    
    public function register() {
        $param['pageNo'] = 2;
        return View::make('company.user.register')->with($param);
    }
    ...
    public function doRegister() {
        ...
        if ($users->count() > 0) {
            ...
            if ($consumers->get()->count() > 0) {
                $alert['msg'] = 'This consumer already registered on our business';
                $alert['type'] = 'info';
            } else {                
                CommonFunctionModel::addConsumer(Session::get('company_id'), $user->id, 0);
                $alert['msg'] = 'This consumer has been registered successfully on our business';
                $alert['type'] = 'success';                
            }
        } else {
            $user = new UserModel;
    ...
    public function detail($id) {
        ...
        return View::make('company.user.detail')->with($param);
    }
    ...
    public function useLoyalty($id, $id2) {
        ...
        $alert['msg'] = 'Loyalty has been used.';
        $alert['type'] = 'success';
        return Redirect::route('company.user.detail', $id2)->with('alert', $alert);        
    }
    ...
    public function doMarketing() {
        ...
        if (($company->count_email - count($userIds) >= 0 && $isEmail) 
                || ($company->count_sms - count($userIds) >= 0 && !$isEmail)) {
            ...
            $alert['msg'] = "You don't have enough credits for sending Message";
            $alert['type'] = 'danger';            
        }
        return Redirect::route('company.user')->with('alert', $alert);        
    }
    ...
    public function asyncAddStamp() {
        ...
        return Response::json(['result' => 'success', 'msg' => 'Stamp has been added successfully.', ], 200);
    }    
