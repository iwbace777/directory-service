...
<div class="portlet box red">
    <div class="portlet-title">
		<div class="caption">
			<i class="fa fa-navicon"></i> Consumer List
		</div>
        <div class="actions">
            ...
            <button class="btn blue" id="js-btn-send-sms">
		        <i class="fa fa-comment-o"></i>&nbsp;Send SMS
	        </button>
	    </div>		
	</div>
    <div class="portlet-body">
        <form method="get" action="{{ URL::route('company.user') }}">
            <div class="row">
                <label class="col-sm-3 text-right form-control-static">Consumer Info: </label>
                <div class="col-sm-4">
                    <input type="text" class="form-control" placeholder="Email / Name" name="keyword" value="{{ $keyword }}">
                </div>
                <div class="col-sm-5">
                    <button class="btn red btn-sm" type="submit"><i class="fa fa-search"></i> Search</button>
                    <a class="btn green btn-sm" href="{{ URL::route('company.user') }}?latest=1"><i class="fa fa-user"></i> Latest 30</a>
                    
                    <a class="btn red btn-sm pull-right" href="{{ URL::route('company.user.register') }}"><i class="fa fa-plus"></i> Register Consumer</a>
                    ...
                    <tr>
                        ...
                        <?php
                            ...
                            foreach ($value->user->availableLoyalties() as $key => $loyalty) {
                                ...
                                $tooltip .= $loyalty->name." : ".$loyalty->count_visit." Stamps Requires";
                            } 
                        ?>
                        <td>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				 <textarea class="form-control" rows="5" placeholder="Write Message Here..." id="js-textarea-description"></textarea>
				 <input type="hidden" id="js-hidden-is-email"/>				 
			</div>
			<div class="modal-footer">
				<button type="button" class="btn default" data-dismiss="modal">Close</button>
				<button type="button" class="btn blue" id="js-btn-submit">Submit</button>
			</div>
		</div>
	</div>
</div>
...
@section('custom-scripts')
    ...
    $("button#js-btn-send-email, button#js-btn-send-sms").click(function() {
        ...
        if ($(this).attr("id") == "js-btn-send-email") {
            $("h4#js-h4-title").text("Send Email To User");
            $("#js-hidden-is-email").val(1);
        } else {
            $("h4#js-h4-title").text("Send SMS To User");
